...
### 一些碎碎念
测试中发现调用API的方式会使得GPU占用高于直接llama.cpp。原本希望通过API的形式实现开一个后端同时跑多个任务，但实际上24G显存依旧不完全够这么玩，应该是其他ocr啥的占用到了，调整一部分layer到cpu上又会让效率比较残念，所以最后我是没真的用server版本。只能说希望这个项目原作者能早日支持多任务并行了。
